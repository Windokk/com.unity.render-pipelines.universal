...
namespace UnityEngine.Rendering.Universal
{
    ...
    public class ShadowCaster2D : ShadowCasterGroup2D, ISerializationCallbackReceiver
    {
        ...
        public void UpdateShadows()
        {
            ...
            for (int i = 0; i < m_ShapePath.Length - 1; i++)
            {
                indices.Add(i);
                indices.Add(i + 1);
            }
            ...
            indices.Add(m_ShapePath.Length - 1);
            indices.Add(0);
            var indicesArray = new NativeArray<int>(indices.ToArray(), Allocator.Temp);
            NativeArray<Vector3> verticesArray = new NativeArray<Vector3>(m_ShapePath, Allocator.Temp);
            m_ShadowMesh.SetShapeWithLines(verticesArray, indicesArray, true);
